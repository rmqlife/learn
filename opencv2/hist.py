...
def hist_curve(im):
    h = np.zeros((300,256,3)) #background in black
    if len(im.shape) == 2:
        color = [(255,255,255)]
    elif im.shape[2] == 3:
        color = [ (255,0,0),(0,255,0),(0,0,255) ]
    for ch, col in enumerate(color): # 'ch':channel number,0,1,2 'col':color
...
def hist_lines(im):
    ...
    hist_item = cv2.normalize(hist_item,alpha=0,beta=255,norm_type=cv2.NORM_MINMAX)
    hist=np.int32(np.around(hist_item))
